...
calc_paprica_path_metrics <- function(dataset, possible_path, in_db) {
  ...
  paprica_pathabun <- paprica_pathabun[which(rownames(paprica_pathabun) %in% possible_pathways), ]
  ...
  mgs_pathabun <- mgs_pathabun[which(rownames(mgs_pathabun) %in% possible_pathways), ]
  ...
  paprica_pathabun <- add_missing_funcs(paprica_pathabun, possible_pathways)
  mgs_pathabun <- add_missing_funcs(mgs_pathabun, possible_pathways)
...
# Read in possible pathways output by HUMAnN2
possible_pathways <- read.table("possible_path/humann2_path.txt", header=FALSE, stringsAsFactors = FALSE)$V1
...
for(d in datasets) {
  ...
  paprica_path_performance[[d]] <- calc_paprica_path_metrics(dataset=d, possible_path=possible_pathways, in_db=pathabun)
}
