...
hmp2_val_nsti <- read.table("/home/gavin/projects/hmp2_ibd_working/16S/April2018_redownload/picrust2_full_output/marker_nsti_predicted.tsv",
                            header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

boxplot(hmp2_val_nsti$metadata_NSTI, ylab="NSTI", ylim=c(0, 2.25), col="grey")
abline(h=2, lty=2, lwd=2)

# Read in NSTI values.
hmp_nsti <- read.table("hmp/16S/picrust2_pipeline/picrust2_full_output/marker_nsti_predicted.tsv",
                       header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

mammal_nsti <- read.table("iGEM//16S/picrust2_pipeline/picrust2_full_output/marker_nsti_predicted.tsv",
                       header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

blueberry_nsti <- read.table("blueberry/16S/picrust2_full_output_pipeline/marker_nsti_predicted.tsv",
                        header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

ocean_nsti <- read.table("ocean/16S/picrust2_pipeline/picrust2_full_output/marker_nsti_predicted.tsv",
                        header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

# Read in weighted NSTI values.
hmp_nsti_weighted <- read.table("hmp/16S/picrust2_pipeline/picrust2_full_output/KO_metagenome_out/weighted_nsti.tsv",
                       header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

mammal_nsti_weighted <- read.table("iGEM//16S/picrust2_pipeline/picrust2_full_output/KO_metagenome_out/weighted_nsti.tsv",
                        header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

blueberry_nsti_weighted <- read.table("blueberry//16S/picrust2_full_output_pipeline/KO_metagenome_out/weighted_nsti.tsv",
                        header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)

ocean_nsti_weighted <- read.table("ocean/16S/picrust2_pipeline/picrust2_full_output/KO_metagenome_out/weighted_nsti.tsv",
                         header=T, sep="\t", stringsAsFactors = FALSE, row.names=1)
...
combined_nsti_id <- rbind(hmp_nsti_id, mammal_nsti_id, ocean_nsti_id, blueberry_nsti_id)
...
combined_nsti_weighted <- rbind(hmp_nsti_weighted, mammal_nsti_weighted, ocean_nsti_weighted, blueberry_nsti_weighted)
...
plot_grid(percent_id_boxplots, cropped_nsti_boxplots, weighted_nsti_boxplots, labels = c("A", "B", "C"), align="h", axis="b")

# Kruskal-Wallis test for significant differences in NSTI values.
kruskal.test(dataset ~ nsti, data=combined_nsti_id)
# Kruskal-Wallis chi-squared = 6529.7, df = 5672, p-value = 7.628e-15
